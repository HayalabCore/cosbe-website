    ...
    <div className="space-y-2.5">
      {preview && (
        <div className="relative aspect-video w-full rounded-lg overflow-hidden bg-slate-100 border border-slate-200">
          <Image src={preview} alt={block.alt || 'Preview'} fill className="object-cover" sizes="600px" />
          <button
            type="button"
      ...
      {!preview && (
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 py-8 gap-3">
          ...
          <ImageUpload supabase={supabase} articleId={draftId} label="Upload image" onUploaded={(url) => onChange({ ...block, url })} />
        </div>
      )}
      <input
        className={INPUT_CLS}
