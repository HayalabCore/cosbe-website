    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="sticky top-0 z-20 flex items-center gap-3 px-4 h-14 bg-white border-b border-slate-200 shadow-sm lg:top-0">
        ...
        <p className="flex-1 min-w-0 text-sm font-medium text-slate-700 truncate hidden sm:block">
          {title || (articleId ? 'Edit post' : 'New post')}
        </p>
        ...
        >
          ...
          {isPublished ? 'Published' : status === 'archived' ? 'Archived' : 'Draft'}
        </span>
      ...
      {tab === 'preview' ? (
        <div className="w-full min-w-0 px-4 md:px-8 lg:px-10 py-10">
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6 md:p-10 lg:p-12">
            <h1 className="text-3xl font-bold text-slate-900 mb-6">{title || 'Untitled'}</h1>
        ...
        <div className="flex flex-1 items-start min-w-0">
          ...
          <div className="flex-1 min-w-0 w-full px-4 md:px-8 lg:px-10 py-8">
            ...
            <textarea
              ...
              placeholder="Add a short excerpt… (optional)"
              rows={2}
            ...
            <div className="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
              <span className="text-[11px] font-medium text-slate-400">{words} words</span>
              <span className="text-slate-200">·</span>
              <span className="text-[11px] font-medium text-slate-400">{readingMins} min read</span>
              <span className="text-slate-200">·</span>
              <span className="text-[11px] font-medium text-slate-400">{blocks.length} block{blocks.length !== 1 ? 's' : ''}</span>
            </div>
          ...
          <div className="xl:hidden w-full px-4 pb-8 mt-4 border-t border-slate-100 pt-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-4">Post settings</p>
